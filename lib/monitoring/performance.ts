...
function setupPerformanceObservers() {
  ...
  try {
    ...
    const layoutShiftObserver = new PerformanceObserver((entries) => {
      ...
      logger.info('Layout shift metric', {
        metric,
        url: window.location.href,
        pathname: window.location.pathname
      }, ['performance', 'layout']);
...
export function measureExecutionTime<T>(fn: () => T, name: string): T {
  ...
  logger.debug(`Execution time for ${name}`, {
    metric,
...
export async function measureAsyncExecutionTime<T>(
  ...
  try {
    ...
    logger.debug(`Async execution time for ${name}`, {
      metric,
      url: typeof window !== 'undefined' ? window.location.href : undefined,
      pathname: typeof window !== 'undefined' ? window.location.pathname : undefined
    }, ['performance', 'execution-time']);
    ...
    logger.warn(`Error in async execution for ${name}`, {
      metric,
      error,
      url: typeof window !== 'undefined' ? window.location.href : undefined,
      pathname: typeof window !== 'undefined' ? window.location.pathname : undefined
...
export function usePerformanceMonitoring(componentName: string) {
  ...
  return {
    ...
    reportRenderTime: () => {
      ...
      logger.debug(`Render time for ${componentName}`, {
        metric,
        url: typeof window !== 'undefined' ? window.location.href : undefined,
        pathname: typeof window !== 'undefined' ? window.location.pathname : undefined
