...
import { withDatabaseHandler, successResponse, HttpStatus } from '../../../lib/db/nextjs-utils';
import {
...
import { Property, CreatePropertyInput, UpdatePropertyInput } from '../../../lib/types/property';
...
const createPropertySchema = z
  .object({
    ...
    squareFeet: z
      .number()
      .positive({ message: 'Square feet must be a positive number' })
      .optional(),
 ...
 */
const updatePropertySchema = createPropertySchema.omit({ uploadId: true }).partial().extend({
...
export const GET = withDatabaseHandler(
  withAuth(
    async (req: NextRequest, user) => {
      ...
      if (id && uploadId) {
        return ApiErrors.invalidParameter('query parameters', 'id OR uploadId');
      }
