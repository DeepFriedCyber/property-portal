...
export const ApiErrors = {
  ...
  missingParameter: (paramName: string) => {
    ...
    if (paramName === 'id') {
      ...
      customMessage = "Either 'id' (for a single property) or 'uploadId' (for multiple properties) is required";
    } else {
  ...
  invalidParameter: (paramName: string, reason?: string) => {
    ...
    if (paramName === 'query parameters' && reason?.includes('id OR uploadId')) {
      message = "API request conflict: You cannot specify both 'id' and 'uploadId' parameters simultaneously. Please use only one of these parameters per request.";
